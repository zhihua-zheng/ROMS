...
#if defined CIRCLE_BOX
      dx=Xsize/REAL(Lm(ng),r8)
      dy=Esize/REAL(Mm(ng),r8)
      DO j=Jmin,Jmax
        DO i=Imin,Imax
          xp(i,j)=dx*REAL(i-1,r8) - XSIZE/2.0_r8
          xr(i,j)=dx*(REAL(i-1,r8)+0.5_r8) - XSIZE/2.0_r8
          xu(i,j)=xp(i,j)
          xv(i,j)=xr(i,j)
          yp(i,j)=dy*REAL(j-1,r8) - ESIZE/2.0_r8
          yr(i,j)=dy*(REAL(j-1,r8)+0.5_r8) - ESIZE/2.0_r8
          yu(i,j)=yr(i,j)
          yv(i,j)=yp(i,j)
        END DO
      END DO
#elif defined CIRCLE_POLAR
!
!  Polar coordinates set-up.
!
      hole = 0.1_r8
      dx=Xsize/REAL(Lm(ng),r8)*0.5*(1.0-hole)
      dth=2.0_r8*pi/REAL(Mm(ng),r8)               ! equal azimuthal spacing
      cff=(4.0_r8*pi/(dth*REAL(Mm(ng),r8)))-1.0_r8   ! F
      DO j=Jmin,Jmax
        DO i=Imin,Imax
          r=Xsize*0.5*hole + dx*REAL(i-1,r8)
          theta=-pi+dth*REAL(j-1,r8)
          xp(i,j)=r*COS(theta)
          yp(i,j)=r*SIN(theta)
          r=Xsize*0.5*hole + dx*(REAL(i-1,r8)+0.5)
          theta=-pi+dth*(REAL(j-1,r8)+0.5)
          xr(i,j)=r*COS(theta)
          yr(i,j)=r*SIN(theta)
          xu(i,j)=xp(i,j)
          yu(i,j)=yr(i,j)
...
!
      DO j=J_RANGE
        DO i=I_RANGE
          r=Xsize*0.5*hole + dx*(REAL(i-1,r8)+0.5_r8)
          wrkX(i,j)=1.0_r8/dx
          wrkY(i,j)=1.0_r8/(r*dth)
        END DO
      END DO
#undef J_RANGE
#undef I_RANGE
...
!-----------------------------------------------------------------------
!
#if defined CIRCLE_POLAR
      DO j=JstrR-1,JendR+1
        DO i=IstrR,IendR
          theta=-pi+dth*(REAL(j-1,r8)+0.5_r8)
          angler(i,j)=theta
        END DO
      END DO
#else
