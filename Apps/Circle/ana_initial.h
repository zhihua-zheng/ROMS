...
!
      ...
      real(r8) :: FF, vt, vr, depth, c0, theta2
...
! u-velocity points
	    ...
	    theta2 = theta - 0.5*(angler(i,j)+angler(i-1,j))
            ...
            ubar(i,j,1) = vr*cos(theta2) - vt*sin(theta2)
#ifdef MASKING
          ELSE
            ubar(i,j,1) = 0.0
          END IF
...
! v-velocity points
	    ...
	    theta2 = theta - 0.5*(angler(i,j)+angler(i,j-1))
            ...
            vbar(i,j,1) = vr*sin(theta2) + vt*cos(theta2)
#ifdef MASKING
          ELSE
            vbar(i,j,1) = 0.0
          END IF
